            ...
            <div class="hidden md:flex items-center gap-4">
                @guest
                    ...
                    <a href="{{ route('user.cart.index') }}" class="relative text-white hover:text-blue-100 dark:hover:text-slate-200"
                        x-data="{ count: 0 }"
                        x-init="count = JSON.parse(localStorage.getItem('guest_cart') || '[]').reduce((sum, item) => sum + parseInt(item.quantity), 0)"
                    ...
                    <a href="{{ route('login') }}" class="text-white hover:text-blue-100 dark:hover:text-slate-200 font-medium">
                        Login
                    </a>
        ...
        <div x-show="mobileMenuOpen" 
            ...
            @guest
                <a href="{{ route('user.cart.index') }}" class="block px-4 py-2 text-white hover:bg-blue-700 dark:hover:bg-slate-700 rounded"
                    ...
                    x-init="count = JSON.parse(localStorage.getItem('guest_cart') || '[]').reduce((sum, item) => sum + parseInt(item.quantity), 0)"
                    @cart-count-updated.window="count = $event.detail">
                    Cart <span x-show="count > 0">(<span x-text="count"></span>)</span>
                </a>
                <a href="{{ route('login') }}" class="block px-4 py-2 text-white hover:bg-blue-700 dark:hover:bg-slate-700 rounded font-medium">
                    Login
                </a>
                <a href="{{ route('register') }}" class="block px-4 py-2 text-white hover:bg-blue-700 dark:hover:bg-slate-700 rounded font-medium">
                    Sign Up
                </a>
            @else
                ...
                <form action="{{ route('logout') }}" method="POST" class="px-4">
                    @csrf
                    <button type="submit" class="w-full text-left py-2 text-white hover:text-blue-100 dark:hover:text-slate-200">
                        Logout
                    </button>
                </form>
            @endguest
