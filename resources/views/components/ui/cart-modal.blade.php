    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl max-h-[90vh] overflow-hidden">
        <div class="bg-amber-50 rounded-lg shadow-xl m-4">
            ...
            <div class="flex justify-between items-center p-6 border-b border-amber-200">
                ...
                <button id="close-user-cart-modal" class="text-gray-500 hover:text-gray-700 transition-colors duration-200 text-xl font-semibold">
                    Close
                </button>
            </div>
            ...
            <div class="p-6 max-h-[60vh] overflow-y-auto">
                ...
                @if($cartItems->count() > 0)
                    <div class="space-y-4">
                        @foreach($cartItems as $cartItem)
                            ...
                            <div class="bg-white rounded-lg border border-amber-100 p-4 hover:shadow-md transition-shadow duration-200">
                                <!-- Product Header -->
                                <div class="flex justify-between items-start mb-3">
                                    <h3 class="text-lg font-semibold text-gray-800">{{ $product->name }}</h3>
                                    <span class="text-lg font-bold text-green-600">{{ number_format(($product->price/100), 2) }} NPR</span>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mb-3 text-sm">
                                    <div class="flex items-center">
                                        ...
                                        <span class="bg-amber-100 px-3 py-1 rounded-full font-semibold">{{ $cartItem->quantity }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <span class="font-medium text-gray-600 mr-2">Total:</span>
                                        <span class="font-semibold text-gray-800">{{ number_format($itemTotal, 2) }} NPR</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-end space-x-3 pt-3 border-t border-gray-100">
                                    ...
                                    <x-ui.delete-modal action="{{ route('users.carts.destroy', ['cart' => $cartItem->id, 'user' => auth()->id()]) }}" />
                                    <form action="{{ route('cart-items.destroy', $cartItem->id) }}" 
                ...
                @else
                    ...
                    <div class="text-center py-12">
                        ...
                        <button class="px-6 py-3 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors duration-200 font-medium">
                            Continue Shopping
                        </button>
                    </div>
                @endif
