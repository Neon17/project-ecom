<x-layouts.admin>
    ...
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8 bg-gray-100 dark:bg-slate-800 min-h-screen rounded-lg shadow-md mt-8">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md p-6 mb-8 flex justify-between items-center">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Category #{{ $category->id }}</h1>
        ...
        @if ($category)
            <form action="{{route('admin.categories.update', $category)}}" method="post" class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-gray-200 dark:border-slate-700 p-8 space-y-6">
                ...
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200 mb-6 pb-4 border-b border-gray-200 dark:border-slate-700">Category Details</h2>

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 mb-2">Name:</label>
                    <input type="text" name="name" value="{{$category->name}}" id="name"
                        class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200">
                    @error('name')
                ...
                <div>
                    <label for="slug" class="block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 mb-2">Slug:</label>
