        ...
        <form action="{{ route('admin.orders.update', [$order->user_id, $order->id]) }}"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    @foreach (['pending', 'processed', 'completed', 'cancelled'] as $status)
                        <label
                            class="flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all {{ $order->status == $status ? 'bg-blue-50 border-blue-500 ring-2 ring-blue-200' : 'border-gray-200 hover:bg-gray-50 hover:border-gray-300' }}">
                            <input type="radio" name="status" value="{{ $status }}"
                                {{ $order->status == $status ? 'checked' : '' }}
                                class="mr-3 text-blue-600 focus:ring-blue-500">
                            <span class="text-gray-800 capitalize font-medium">{{ $status }}</span>
                        </label>
                    @endforeach
                </div>
                @error('status')
                        ...
                        <div class="border border-gray-200 rounded-xl p-5 bg-gray-50">
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                                <div>
                                    ...
                                    <p class="p-3 bg-white border border-gray-200 rounded-lg text-gray-900 font-medium">
                                        {{ $item->product->name }}
                                    </p>
                                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Street Address 2</label>
