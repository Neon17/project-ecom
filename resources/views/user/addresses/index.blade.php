<x-layouts.user>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gray-50 dark:bg-slate-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 flex flex-col items-center justify-center text-center hover:bg-gray-100 dark:hover:bg-slate-700 dark:bg-slate-800 transition-colors cursor-pointer"
             onclick="document.getElementById('add-address-modal').classList.remove('hidden')">
            <div class="w-12 h-12 rounded-full bg-gray-200 dark:bg-slate-700 flex items-center justify-center mb-4 text-gray-500 dark:text-gray-400 dark:text-gray-500">
                <i class="fas fa-plus text-xl"></i>
            </div>
            <h3 class="font-bold text-gray-800 dark:text-gray-200">Add New Address</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">Add a new shipping destination</p>
        </div>

        @foreach($addresses as $address)
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-6 relative group">
                <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                    <button onclick="editAddress({{ $address }})" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 p-1">
                ...
                <div class="flex items-start mb-4">
                    <div class="p-2 rounded bg-blue-50 text-blue-600 dark:text-blue-400 mr-3">
                        <i class="fas fa-map-marker-alt"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-gray-800 dark:text-gray-200">Address #{{ $loop->iteration }}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{{ $address->country }}</p>
                    </div>
                </div>

                <div class="text-gray-600 dark:text-gray-300 dark:text-gray-600 text-sm space-y-1">
