                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-800 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-gray-200">Unpaid</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                NPR {{ number_format($order->total, 2) }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('user.orders.show', $order) }}"
                                    class="text-blue-600 dark:text-blue-400 hover:text-blue-800 font-medium text-sm">View Details</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400 dark:text-gray-500">
                                No orders found. <a href="{{ route('products.index') }}"
                                    class="text-blue-600 dark:text-blue-400 hover:underline">Start shopping</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
