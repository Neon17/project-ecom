    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
                    <h2 class="text-lg font-bold text-gray-800 dark:text-gray-200">Order Items</h2>
                </div>
                    ...
                    @foreach($order->orderItems as $item)
                        <div class="p-6 flex items-center">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">Quantity: {{ $item->quantity }}</p>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-gray-900 dark:text-white">NPR {{ number_format($item->amount_per_item, 2) }}</p>
                                <p class="text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">Total: NPR {{ number_format($item->amount_per_item * $item->quantity, 2) }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                <div class="bg-gray-50 dark:bg-slate-800 px-6 py-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-slate-700">
                        ...
                        <span class="text-lg font-bold text-blue-600 dark:text-blue-400">NPR {{ number_format($order->total, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-200 mb-4">Order Status</h2>
                ...
                @if($order->status === 'pending' && (!$order->payment || $order->payment->status !== \App\Enums\PaymentStatusEnum::Completed))
                    <a href="{{ route('orders.pay', $order) }}" class="block w-full bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white text-center font-semibold py-2 rounded-lg transition-colors">
                        Pay Now
                    </a>
                @endif
            </div>
            ...
            <!-- Payment Info -->
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-200 mb-4">Payment Info</h2>
                @if($order->payment)
                    <div class="mb-2">
                        <span class="text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">Method:</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ ucfirst($order->payment->payment_method->value) }}</span>
                    </div>
                    <div class="mb-2">
                        <span class="text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">Status:</span>
